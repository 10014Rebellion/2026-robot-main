...
public class FlywheelsSS extends SubsystemBase {
  ...
  public void setFlywheelClosedLoop(Rotation2d pRotsPerS){
    ...
    double bangOutput = 12 * mBangBangController.calculate(getFlywheelRPS().getRotations(), pRotsPerS.getRotations());
    double ffOutput = tFFMultiplier.get() * kFlywheelControlConfig.feedforward().calculate(pRotsPerS.getRotations());
    mLeaderFlywheelIO.setMotorVolts(bangOutput + ffOutput);
    mFollowerFlywheelIO.enforceFollower();
  }
